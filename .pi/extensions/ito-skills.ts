...
export default function itoSkills(pi: ExtensionAPI) {
  ...
  const getBootstrapContent = (): string => {
    try {
      // Use "opencode" tool type for bootstrap since Pi's tool model is
      // closest to OpenCode (Read/Write/Edit/Bash tools, skill loading).
      const bootstrap = execFileSync(
        "ito",
        ["agent", "instruction", "bootstrap", "--tool", "opencode"],
        {
          cwd: directory,
          encoding: "utf8",
