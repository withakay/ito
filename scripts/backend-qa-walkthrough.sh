...
Usage:
  scripts/backend-qa-walkthrough.sh <command>
...
Generated by scripts/backend-qa-walkthrough.sh
...
require_server() {
  if ! server_running; then
    printf 'Backend QA server is not running. Start it with: scripts/backend-qa-walkthrough.sh start\n' >&2
    exit 1
  fi
}
...
walk() {
  ...
  printf 'Stop it with: scripts/backend-qa-walkthrough.sh stop\n'
}
