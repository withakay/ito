...
- Established pre-apply checks for nearby codemap.md files
- Defined guide refresh, verification, and post-structure-update behavior

**Files:**
- codemap.md
- ito-rs/codemap.md
- ito-rs/crates/codemap.md
- source-guide.json

**Flow:**
check nearby codemap.md files -> refresh missing/stale guides -> read guides for orientation -> verify claims against source -> update affected guides after structural changes
...
## Narrative
### Structure
The workflow spans a root codemap, an ito-rs-level guide, a crates-level guide, and per-crate codemap.md files, with freshness tracked in source-guide.json.
...
### Rules
Before implementing an Ito apply change, agents should check for nearby codemap.md files, use the source-guide skill to set up or refresh missing/stale guides, read guides as orientation rather than authority, verify important claims against source, and update affected guides after structural changes.

## Facts
- **workflow**: Ito now uses the source-guide skill as its code map/code atlas workflow. [project]
- **pre_change_check**: Before implementing an Ito apply change, agents should check for nearby codemap.md files. [convention]
...
- **guide_coverage**: The repository includes a root codemap.md, ito-rs/codemap.md, ito-rs/crates/codemap.md, and one codemap.md per Rust crate. [project]
