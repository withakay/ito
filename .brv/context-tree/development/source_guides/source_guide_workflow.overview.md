## Key points
- Ito adopts `codemap.md` files as a **code map / code atlas** workflow for apply changes.
- Before making an Ito apply change, agents should **check for nearby `codemap.md` files** and refresh any that are missing or stale.
...
- The workflow spans multiple levels of guidance: a root guide, `ito-rs`-level guides, crates-level guides, and per-crate `codemap.md` files.
...
- **Entities**: `codemap.md`, `ito-rs/codemap.md`, `ito-rs/crates/codemap.md`, `source-guide.json`.
