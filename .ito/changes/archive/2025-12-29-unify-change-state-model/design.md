...
|--------|---------|---------|
| **Task Progress** | Track implementation work | `ito dashboard`, `ito list` |
| **Artifact Progress** | Track planning/spec work | `ito status`, `ito next` |
...
async function validateChangeExists(changeName: string, projectRoot: string): Promise<string> {
  const changePath = path.join(projectRoot, 'ito', 'changes', changeName);
  ...
  if (!fs.existsSync(changePath) || !fs.statSync(changePath).isDirectory()) {
    ...
    const entries = await fs.promises.readdir(
      path.join(projectRoot, 'ito', 'changes'),
      { withFileTypes: true }
    );
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: ito new change <name>');
    }
