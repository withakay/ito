...
Result: `ito create change foo && ito dashboard` shows `foo` as "Completed" when it has no content.
...
Result: `ito status --change foo` says "not found" even though the directory exists.
...
|---------|--------|-------|
| `ito view` | Empty = "Completed" | Empty = "Draft" |
| `ito status --change X` | Requires proposal.md | Works on any directory |
| `ito validate X` | Requires proposal.md | Unchanged (still requires it) |

## Breaking Changes

### Minimal Breaking Change

1. **`ito view` output**: Empty changes move from "Completed" section to new "Draft" section

### Non-Breaking

- Active/Completed semantics unchanged (still task-based)
- `getActiveChangeIds()` unchanged
- `ito validate` unchanged
...
|---------|-----------------------------|-----------------------------|----------------|
| `ito view` | No (reads dirs directly) | Yes → Draft section | **Yes** |
| `ito list` | No (reads dirs directly) | Yes (shows "No tasks") | No |
| `ito status/next/instructions` | Yes | Yes | **Yes** |
| `ito validate` | Yes | No (can't validate empty) | No |
| `ito show` | Yes | No (nothing to show) | No |
| Tab completions | Yes | Future enhancement | No |

## Success Criteria

1. `ito new change foo && ito view` shows `foo` in "Draft" section
1. `ito new change foo && ito status --change foo` works
