...
Ito needs usage analytics to understand adoption and inform product decisions. PostHog provides a privacy-conscious analytics platform suitable for open source projects.
...
```bash
ITO_TELEMETRY=0    # Disable telemetry
...
- Full `ito telemetry` command - Can add later if users request
...
```
Note: Ito collects anonymous usage stats. Opt out: ITO_TELEMETRY=0
```
