...
The system SHALL store and validate per-change metadata in `.ito.yaml` files using a Zod schema.

#### Scenario: Metadata file created with new change

- **WHEN** user runs `ito new change add-feature --schema tdd`
- **THEN** the system creates `.ito.yaml` in the change directory
- **AND** the file contains `schema: tdd` and `created: <YYYY-MM-DD>`

#### Scenario: Metadata validated on read

- **WHEN** the system reads `.ito.yaml`
...
- **WHEN** `.ito.yaml` contains invalid YAML or missing required fields
- **THEN** the system displays a Zod validation error with details

#### Scenario: Missing metadata file

- **WHEN** a change directory has no `.ito.yaml` file
...
- **WHEN** user runs `ito status --change <id>` without `--schema`
- **AND** the change has `.ito.yaml` with `schema: tdd`
- **THEN** the system uses the `tdd` schema

#### Scenario: Explicit schema overrides metadata

- **WHEN** user runs `ito status --change <id> --schema spec-driven`
- **AND** the change has `.ito.yaml` with `schema: tdd`
...
- **AND** the change has no `.ito.yaml` file
