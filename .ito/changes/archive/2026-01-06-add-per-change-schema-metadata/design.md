...
The metadata file (`.ito.yaml`) will be validated with this Zod schema:
...
/**
 * Schema for per-change metadata stored in .ito.yaml
 */
...
**Location:** `ito/changes/<name>/.ito.yaml`
...
- `ito.json` - YAML matches existing schema files
...
const METADATA_FILENAME = '.ito.yaml';
...
1. **`.ito.yaml` metadata** (persisted choice)
...
- Existing changes without `.ito.yaml` continue to work (use default)
- New changes created with `ito new change --schema X` get metadata file

## Open Questions

- Should `ito new change` prompt for schema interactively if not specified? (Leaning no - default is fine)
