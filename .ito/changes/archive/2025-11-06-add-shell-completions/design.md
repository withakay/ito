...
```bash
# After installing: ito completion install bash
ito val<TAB>           # Completes to "ito validate"
ito validate <TAB><TAB>  # Shows: --all --changes --specs --strict --json [change-ids] [spec-ids]
ito show add-<TAB><TAB>  # Shows all changes starting with "add-"
```
...
```zsh
# After installing: ito completion install zsh
ito val<TAB>    # Shows menu with "validate" and "view" highlighted
ito show <TAB>  # Shows menu with all change IDs and spec IDs, categorized
```
...
```fish
# After installing: ito completion install fish
ito val       # Gray suggestion shows "validate" immediately
ito show a    # Real-time suggestions for changes starting with "a"
ito <TAB>     # Shows all commands with descriptions in paged menu
```
...
```powershell
# After installing: ito completion install powershell
ito val<TAB>       # Cycles: validate → view → validate
ito show <TAB>     # Cycles through change IDs one by one
ito <Ctrl+Space>   # Shows IntelliSense menu with all commands
```
...
class CompletionProvider {
  ...
  isItoProject(): boolean {
    // Check for ito/ directory
  }
}
```
...
- Graceful degradation when outside Ito projects
...
│       ├── command-registry.ts    # Single source of truth for Ito commands
...
if (isOhMyZshInstalled()) {
  // Install to ~/.oh-my-zsh/custom/completions/_ito
  // Automatically loaded by Oh My Zsh
} else {
  // Install to ~/.zsh/completions/_ito
...
1. **Not in Ito project** - Skip dynamic completions, only offer static commands
...
1. **Manual Testing**
   ...
   - Tab completion behavior in Ito projects
