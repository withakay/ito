...
Provide a CLI interface for viewing and modifying global Ito configuration. Enables users to manage settings without manually editing JSON files, with support for scripting and automation.
...
- **WHEN** user executes `ito config --help`
...
- **WHEN** user executes `ito config path`
...
- **WHEN** user executes `ito config list --json`
...
- **WHEN** user executes `ito config set <key> true` or `ito config set <key> false`
...
- **WHEN** user executes `ito config set <key> <value> --string`
...
- **WHEN** user executes `ito config set featureFlags.newFlag true`
...
- **WHEN** user executes `ito config reset --all -y`
- **THEN** reset without prompting for confirmation

#### Scenario: Reset without --all flag

- **WHEN** user executes `ito config reset` without `--all`
...
- **WHEN** user executes `ito config set someFutureKey 123`
- **THEN** the value is saved successfully
- **AND** exit with code 0

#### Scenario: Invalid feature flag value rejected

- **WHEN** user executes `ito config set featureFlags.someFlag notABoolean`
...
- **WHEN** user executes `ito config --scope project <subcommand>`
