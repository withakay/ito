...
- **NEW**: `ito status --change <id>` shows artifact completion state
- **NEW**: `ito next --change <id>` shows artifacts ready to create
- **NEW**: `ito instructions <artifact> --change <id>` outputs enriched template
- **NEW**: `ito templates [--schema <name>]` shows resolved template paths
- **NEW**: `ito new change <name>` creates a new change directory
