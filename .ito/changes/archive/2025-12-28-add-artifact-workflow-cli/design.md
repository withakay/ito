...
```
ito status --change <id>
ito next --change <id>
ito instructions <artifact> --change <id>
ito templates [--schema <name>]
ito new change <name>
```
...
ito status                      # error: missing --change
```
...
```bash
ito new change add-feature
```

**Rationale:**

- `ito new <name>` is ambiguous (new what?)
- `ito new change <name>` is clear and extensible
- Can add `ito new spec <name>` later if needed
