...
- Teams already rely on Ito to manage slash-command configurations for Claude Code, Cursor, OpenCode, Codex, Kilo Code, and Windsurf. Excluding GitHub Copilot forces developers to manually maintain Ito prompts in `.github/prompts/`, which leads to drift and undermines Ito's "single source of truth" promise.
...
- Add GitHub Copilot to the `ito init` tool picker with "already configured" detection similar to other editors, wiring an implementation that writes managed Markdown prompt files to `.github/prompts/` with Ito marker blocks.
- Generate three GitHub Copilot prompt files—`ito-proposal.prompt.md`, `ito-apply.prompt.md`, and `ito-archive.prompt.md`—whose content mirrors shared slash-command templates while conforming to Copilot's frontmatter and `$ARGUMENTS` placeholder convention.
- Document GitHub Copilot's repository-based discovery and that Ito writes prompts to `.github/prompts/` with managed blocks.
- Teach `ito update` to refresh existing GitHub Copilot prompts in-place (only when they already exist) in the repository's `.github/prompts/` directory.
