...
- Respect `--no-interactive` and `ITO_INTERACTIVE=0` consistently.
...
- `ito show` exists and prints a helpful hint in non-interactive contexts when no args.
...
- `ito show <change-id> --json --deltas-only` matches `ito change show <id> --json --deltas-only` output.
- `ito show <spec-id> --json --requirements` matches `ito spec show <id> --json --requirements` output.
...
- \[x\] `src/commands/change.ts` → extend `show(changeName?: string, options?: { json?: boolean; requirementsOnly?: boolean; deltasOnly?: boolean; noInteractive?: boolean })`:
  ...
  - Non-interactive fallback: keep current behavior (print available IDs + `ito change list` hint, set `process.exitCode = 1`).
...
- `ito change show` in non-interactive prints list hint and exits non-zero.
- `ito spec show` in non-interactive prints missing-arg error and exits non-zero.
...
- \[x\] Top-level `show` hint (non-interactive no-arg):
  - Lines include: `ito show <item>`, `ito change show`, `ito spec show`, and "Or run in an interactive terminal.".
...
Add tests mirroring existing patterns (non-TTY simulation via `ITO_INTERACTIVE=0`).
...
- \[x\] `test/commands/change.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `ito change show` without args prints available IDs + list hint and non-zero exit.
- \[x\] `test/commands/spec.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `ito spec show` without args prints missing-arg error and non-zero exit.
...
- \[x\] Update `ito/README.md` usage examples to include the new `show` command with type detection and flags.
