...
- **WHEN** executing `ito show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `ito show <item>` or `ito change/spec show`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `ito change show`/`ito spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `ito show --type change <item>`

- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `ito show --type spec <item>`
...
- **WHEN** executing `ito show <item> --json`
...
- **WHEN** showing a change with `ito show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `ito show <spec-id> --requirements`
