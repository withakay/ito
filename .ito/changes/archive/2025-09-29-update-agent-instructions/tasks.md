...
- \[x\] 3.1 Document `ito list` and `ito list --specs` commands
- \[x\] 3.2 Document `ito show` with all flags and interactive mode
- \[x\] 3.3 Document `ito diff [change]` for viewing spec differences
- \[x\] 3.4 Document `ito archive` with --skip-specs option
- \[x\] 3.5 Document `ito validate` with --strict and batch modes
- \[x\] 3.6 Document `ito init` and `ito update` commands
...
- \[x\] 8.2 Run `ito init` to verify CLAUDE.md generation
