...
The system SHALL generate schema-aware apply instructions via `ito instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `ito instructions apply --change <id>`
...
- **WHEN** user runs `ito instructions apply --change <id> --json`
