...
- \[x\] 1.2 Implement a `CodexSlashCommandConfigurator` that writes `.codex/prompts/ito-{proposal,apply,archive}.md`, ensuring the prompt directory exists and wrapping content in Ito markers.
...
- \[x\] 3.1 Update the `ito update` flow to refresh existing Codex prompts without creating new ones when files are missing.
...
- \[x\] 4.2 Add a release note snippet that points Codex users to the generated `/ito-proposal`, `/ito-apply`, and `/ito-archive` commands.
