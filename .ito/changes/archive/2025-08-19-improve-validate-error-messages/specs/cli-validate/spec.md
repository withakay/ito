...
- **THEN** show error "No deltas found" with guidance:
  - Ensure `ito/changes/{id}/specs/` exists with `.md` files
  - Use delta headers: `## ADDED Requirements`, `## MODIFIED Requirements`, `## REMOVED Requirements`, `## RENAMED Requirements`
  - Each requirement must include at least one `#### Scenario:` block
  - Try: `ito change show {id} --json --deltas-only` to inspect what was parsed
...
- Source file path (`ito/changes/{id}/proposal.md`, `.../specs/{cap}/spec.md`)
...
- **THEN** print "Next steps" with 2-3 targeted bullets and suggest `ito change show <id> --json --deltas-only`
