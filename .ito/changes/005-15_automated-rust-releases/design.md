...
- Current state:
  ...
  - Artifact publishing is handled by a GitHub Actions workflow generated by cargo-dist.
...
- GitHub release assets: `/.github/workflows/v-release.yml`
  - Trigger: tag push for `vX.Y.Z` (and PRs run `dist plan` for visibility)
  - Uses cargo-dist to build and upload archives/installers/checksums
  - Notes: cargo-dist also creates/updates the GitHub Release body/title by default
...
- Decision: Use a root `Cargo.toml` virtual workspace that references crates under `ito-rs/`.
  ...
  - Intended shape:
    ...
    - Keep a single authoritative workspace at the git root.
  - Alternative considered:
    - Keep the workspace manifest under `ito-rs/` and rely on `manifest_path` everywhere (rejected: too easy to regress).
...
- Decision: Separate responsibilities between versioning/publishing and artifact distribution.
...
- GitHub Release + binary artifacts
  ...
  - Release workflow files are generated by cargo-dist; avoid manual edits and regenerate via `dist generate` when settings change.
...
- Runners
  - Use GitHub-hosted runners for all jobs (ubuntu/macOS/windows) to reduce self-hosted variance.
