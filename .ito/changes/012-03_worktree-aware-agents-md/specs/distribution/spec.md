...
#### Scenario: Skills with Jinja2 syntax are rendered before installation

- **WHEN** `ito init` or `ito update` installs a skill template containing Jinja2 syntax (`{%` or `{{`)
- **THEN** the system SHALL render the template with the current worktree configuration context before writing to the destination
- **AND** skills without Jinja2 syntax SHALL continue to be copied verbatim
