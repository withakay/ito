...
|------|------------|---------|
| **Change** | A unit of work being planned (feature, refactor, migration) | `ito/changes/add-auth/` |
...
```
1. ${XDG_DATA_HOME}/ito/schemas/<name>/schema.yaml   # Global user override
2. <package>/schemas/<name>/schema.yaml                    # Built-in defaults
```

**Platform-specific paths:**

- Unix/macOS: `~/.local/share/ito/schemas/`
- Windows: `%LOCALAPPDATA%/ito/schemas/`
- All platforms: `$XDG_DATA_HOME/ito/schemas/` (when set)
...
```
1. ${XDG_DATA_HOME}/ito/schemas/<schema>/templates/<artifact>.md  # User override
...
| Responsibility | Approach |
|----------------|----------|
| Create changes | Create dirs under `ito/changes/<name>/` with README |
| Name validation | Enforce kebab-case naming |

**Key Paths:**

```
ito/changes/<name>/   → Change instances with artifacts (project-level)
```
...
| `list` | List all changes | EXISTS (`ito change list`) |
| `new <name>` | Create change | **NEW** (uses `createChange()`) |
| `init` | Initialize structure | EXISTS (`ito init`) |
...
- `ito change list` / `ito change show <id>` / `ito change validate <id>`
- `ito list --changes` / `ito list --specs`
- `ito dashboard`
- `ito init` / `ito archive <change>`
...
**Note:** Legacy commands (`/ito-proposal`, `/ito-apply`, `/ito-archive`) exist in the main project for backward compatibility but are separate from this architecture.
...
│  │  ~/.local/share/ │   │  ito/changes/<name>/      │  │
│  │  ito/       │   │  - proposal.md, design.md      │  │
...
```
ito status --change add-auth       # explicit, works
ito status                          # error: "No change specified"
```
...
```
${XDG_DATA_HOME}/ito/schemas/<name>/schema.yaml   # User override
    ↓ (not found)
...
```
${XDG_DATA_HOME}/ito/schemas/<schema>/templates/<artifact>.md  # User override
    ↓ (not found)
...
- `ito change list/show/validate` - change management
- `ito list --changes/--specs` - listing
- `ito dashboard`
- `ito init` - initialization
...
~/.local/share/ito/           # Unix/macOS ($XDG_DATA_HOME/ito/)
%LOCALAPPDATA%/ito/           # Windows
...
# Or user override: ~/.local/share/ito/schemas/spec-driven/schema.yaml
name: spec-driven
version: 1
