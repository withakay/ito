...
- \[x\] 1.2 Generate `.agent/workflows/ito-proposal.md`, `ito-apply.md`, and `ito-archive.md` with YAML frontmatter restricted to a single `description` field for each stage and wrap the body in Ito markers.
...
- \[x\] 2.1 Detect existing Antigravity workflow files during `ito update` and refresh only the managed body, skipping creation when files are missing.
