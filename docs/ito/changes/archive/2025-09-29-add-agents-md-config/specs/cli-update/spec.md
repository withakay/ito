...
The update command SHALL update Ito instruction files to the latest templates in a team-friendly manner.
...
- **THEN** the command SHALL:
  ...
  - Replace `ito/AGENTS.md` with the latest template (complete replacement)
...
- **WHEN** updating files
- **THEN** completely replace `ito/AGENTS.md` with the latest template
- **AND** create or update the root-level `AGENTS.md` using the Ito markers
- **AND** update only the Ito-managed blocks in **existing** AI tool files using markers
- **AND** use the default directory name `ito`
...
The update command SHALL always update the core Ito files and display an ASCII-safe success message.

#### Scenario: Successful update

- **WHEN** the update completes successfully
- **THEN** replace `ito/AGENTS.md` with the latest template
...
- **AND** display the message: "Updated Ito instructions"
