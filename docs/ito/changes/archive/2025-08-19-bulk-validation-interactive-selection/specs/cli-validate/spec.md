...
- **WHEN** executing `ito validate <item-name>`
...
- **WHEN** executing `ito validate --all`
- **THEN** validate all changes in ito/changes/ (excluding archive)
- **AND** validate all specs in ito/specs/
...
- **THEN** include all change proposals under `ito/changes/`

- **AND** exclude the `ito/changes/archive/` directory

- **WHEN** validating with `--specs`

- **THEN** include all specs that have a `spec.md` under `ito/specs/<id>/spec.md`
...
- **WHEN** executing `ito validate --specs`
- **THEN** validate all specs in ito/specs/
...
- **WHEN** executing `ito validate --all --strict`
...
- **WHEN** executing `ito validate --all --json`
...
- **WHEN** executing `ito validate --all --json` (or `--changes` / `--specs`)
...
- **AND** suggest passing `--type change` or `--type spec`, or using `ito change validate` / `ito spec validate`
...
- **WHEN** executing `ito validate --type change <item>`

- **THEN** treat `<item>` as a change ID and validate it (skipping auto-detection)

- **WHEN** executing `ito validate --type spec <item>`
...
- The CLI SHALL respect `ITO_INTERACTIVE=0` to disable prompts globally.
...
- **WHEN** `ito validate` is executed with `--no-interactive` or with environment `ITO_INTERACTIVE=0`
