...
- \[x\] Create `src/core/completions/command-registry.ts` with `COMMAND_REGISTRY` constant defining all Ito commands, flags, and metadata
...
- \[x\] Implement installation to `~/.oh-my-zsh/custom/completions/_ito` for Oh My Zsh
- \[x\] Implement fallback installation to `~/.zsh/completions/_ito` with `fpath` updates
...
- \[x\] Implement dynamic completion helper functions in Zsh generator (`_ito_complete_changes`, `_ito_complete_specs`, `_ito_complete_items`)
...
- [ ] Test dynamic change/spec ID completion in real Ito projects
- [ ] Verify completion cache behavior (2-second TTL)
- [ ] Test behavior outside Ito projects (should skip dynamic completions)
- \[x\] Update `ito --help` output to include completion command (automatically done via Commander)
