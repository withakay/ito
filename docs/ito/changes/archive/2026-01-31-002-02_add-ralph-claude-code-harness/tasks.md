...
- Validation: run `make test` and `node bin/ito.js validate 002-02_add-ralph-claude-code-harness --strict`

## Wave 1: Spec + CLI Surface

1. Define delta spec updates for `cli-ralph` (claude-code harness)
   - Files: `.ito/changes/002-02_add-ralph-claude-code-harness/specs/cli-ralph/spec.md`
   - Verify: `node bin/ito.js validate 002-02_add-ralph-claude-code-harness --strict`
   - Status: ⬜
...
1. Register harness in `ito ralph`
   ...
   - Verify: `node bin/ito.js ralph --help` shows `claude-code`
   - Status: ⬜
