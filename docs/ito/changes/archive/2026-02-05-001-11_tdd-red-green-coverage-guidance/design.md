...
Ito provides (1) template assets installed by `ito init`/`ito update`, and (2) instruction artifacts generated by `ito agent instruction <artifact>`. Today, testing guidance exists but is not consistently emphasized at the proposal/apply boundary, and coverage expectations are implicit rather than first-class and configurable.
...
- Existing projects can optionally set overrides in `.ito/config.json`, `.ito.json`, or `ito.json`.
