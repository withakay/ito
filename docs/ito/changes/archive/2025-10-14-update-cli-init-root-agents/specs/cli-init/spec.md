...
The command SHALL configure AI coding assistants with Ito instructions using a grouped selection experience so teams can enable native integrations while always provisioning guidance for other assistants.
...
`ito init` SHALL treat extend mode without new native tool selections as a successful refresh.

#### Scenario: Allowing empty extend runs

- **WHEN** Ito is already initialized and the user selects no additional natively supported tools
...
`ito init` SHALL always scaffold the root-level `AGENTS.md` hand-off so every teammate finds the primary Ito instructions.
