...
The `global-config` spec defines how Ito reads/writes `config.json`, but users currently must edit it by hand. This command provides a CLI interface to that config.
...
```bash
ito config get featureFlags              # Returns object
ito config get featureFlags.experimental # Returns nested value
ito config set featureFlags.newFlag true
```
...
- Raw output enables piping: `VAR=$(ito config get key)`
