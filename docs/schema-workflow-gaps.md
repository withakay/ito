...
This document analyzes the complete user journey for working with schemas in Ito, identifies gaps, and proposes a phased solution.
...
| Change creation | `ito create change <name>` — no schema binding |
...
mkdir -p ~/.local/share/ito/schemas/my-workflow/templates

# Step 3: Find the npm package to copy defaults
npm list -g ito --parseable
# Output varies by package manager:
#   npm: /usr/local/lib/node_modules/ito
#   pnpm: ~/.local/share/pnpm/global/5/node_modules/ito
#   volta: ~/.volta/tools/image/packages/ito/...
#   yarn: ~/.config/yarn/global/node_modules/ito

# Step 4: Copy files
cp -r <package-path>/schemas/spec-driven/* \
      ~/.local/share/ito/schemas/my-workflow/
...
- No diff capability when upgrading ito
...
```yaml
# ito/changes/add-auth/change.yaml
schema: tdd
created: 2025-01-15T10:30:00Z
description: Add user authentication system
```

Binds a specific schema to a change. Created automatically by `ito create change`.

### Schema Resolution Order

```
1. ./ito/schemas/<name>/                    # Project-local
2. ~/.local/share/ito/schemas/<name>/       # User global (XDG)
...
3. ito/config.yaml defaultSchema   # Project default
...
ito create change add-auth
# Creates ito/changes/add-auth/change.yaml:
#   schema: spec-driven
#   created: 2025-01-15T10:30:00Z

# Explicit schema for this change
ito create change add-auth --schema tdd
...
ito status --change add-auth --schema spec-driven
...
ito schema copy spec-driven my-workflow
# Created ./ito/schemas/my-workflow/
# Edit schema.yaml and templates/ to customize

# Copy to global (user-level override)
ito schema copy spec-driven --global
# Created ~/.local/share/ito/schemas/spec-driven/

# See where a schema resolves from
ito schema which spec-driven
# ./ito/schemas/spec-driven/ (project)
# or: ~/.local/share/ito/schemas/spec-driven/ (user)
# or: /usr/local/lib/node_modules/ito/schemas/spec-driven/ (built-in)
...
# Removes ./ito/schemas/spec-driven/ (or --global for user dir)
```
...
- Create `change.yaml` when running `ito create change`
...
- Optional: `ito migrate` to add `change.yaml` to existing changes
...
- Add `./ito/schemas/` to resolution order (first priority)
- `ito schema copy <name> [new-name]` creates in project by default
- `--global` flag for user-level XDG directory
- Teams can commit `ito/schemas/` to repo

**Resolution order:**

```
1. ./ito/schemas/<name>/           # Project-local (NEW)
2. ~/.local/share/ito/schemas/<name>/  # User global
3. <npm-package>/schemas/<name>/        # Built-in
```
...
```bash
ito schema list              # Show available schemas with sources
ito schema which <name>      # Show resolution path
ito schema copy <name> [to]  # Copy for customization
ito schema diff <name>       # Compare with built-in
ito schema reset <name>      # Remove override
ito schema validate <name>   # Validate schema.yaml structure
```
...
- Add `ito/config.yaml` with `defaultSchema` field
- `ito init` prompts for schema selection
