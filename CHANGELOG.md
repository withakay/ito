...
- eb152eb: ### New Features
  ...
  - **Continue IDE support** – Ito now generates slash commands for [Continue](https://continue.dev/), expanding editor integration options alongside Cursor, Windsurf, Claude Code, and others
  - **Shell completions for Bash, Fish, and PowerShell** – Run `ito completion install` to set up tab completion in your preferred shell
  - **`/ito-explore` command** – A new thinking partner mode for exploring ideas and investigating problems before committing to changes
  ...
  - Added optional anonymous usage statistics to help understand how Ito is used. This is **opt-out** by default – set `ITO_TELEMETRY=0` or `DO_NOT_TRACK=1` to disable. Only command names and version are collected; no arguments, file paths, or content. Automatically disabled in CI environments.
...
- 8dfd824: Add OPSX experimental workflow commands and enhanced artifact system
  ...
  **New Commands:**
  ...
  - `/ito-ff-change` - Fast-forward through artifact creation, generating all needed artifacts in one go
  - `/ito-sync-specs` - Sync delta specs from a change to main specs
  - `/ito-archive-change` - Archive completed changes with smart sync check
  ...
  **Artifact Workflow Enhancements:**
  ...
  - Per-change schema metadata via `.ito.yaml` files
...
- 2e71835: ### New Features
  ...
  - Add `ito config` command for managing global configuration settings
...
- Add Continue slash command support so `ito init` can generate `.continue/prompts/ito-*.prompt` files with MARKDOWN frontmatter and `$ARGUMENTS` placeholder, and refresh them on `ito update`.

- Add Antigravity slash command support so `ito init` can generate `.agent/workflows/ito-*.md` files with description-only frontmatter and `ito update` refreshes existing workflows alongside Windsurf.
...
- 4758c5c: Add support for new AI tools with native slash command integration
  ...
  - **Gemini CLI**: Add native TOML-based slash command support for Gemini CLI with `.gemini/commands/ito/` integration
...
- 8386b91: Add support for new AI assistants and configuration improvements
  ...
  - fix: recreate missing ito template files in extend mode
...
- 082abb4: Add factory function support for slash commands and non-interactive init options
  ...
  - **Non-interactive init options**: Added `--tools`, `--all-tools`, and `--skip-tools` CLI flags to `ito init` for automated initialization in CI/CD pipelines while maintaining backward compatibility with interactive mode

## 0.11.0

### Minor Changes

- 312e1d6: Add Amazon Q Developer CLI integration. Ito now supports Amazon Q Developer with automatic prompt generation in `.amazonq/prompts/` directory, allowing you to use Ito slash commands with Amazon Q's @-syntax.
...
- 2ae0484: Fix cross-platform path handling issues. This release includes fixes for joinPath behavior and slash command path resolution to ensure Ito works correctly across all platforms.

## 0.9.1

### Patch Changes

- 8210970: Fix Ito not working on Windows when Codex integration is selected. This release includes fixes for cross-platform path handling and normalization to ensure Ito works correctly on Windows systems.
...
- Add GitHub Copilot slash command support. Ito now writes prompts to `.github/prompts/ito-{proposal,apply,archive}.prompt.md` with YAML frontmatter and `$ARGUMENTS` placeholder, and refreshes them on `ito update`.

## 0.8.1

### Patch Changes

- d070d08: Fix CLI version mismatch and add a release guard that validates the packed tarball prints the same version as package.json via `ito --version`.
...
- Add Codex slash command support. Ito now writes prompts directly to Codex's global directory (`~/.codex/prompts` or `$CODEX_HOME/prompts`) and refreshes them on `ito update`.

## 0.7.0

### Minor Changes

- Add native Kilo Code workflow integration so `ito init` and `ito update` manage `.kilocode/workflows/ito-*.md` files.
...
- Add Ito change proposals for CLI improvements and enhanced user experience
...
- Enhance `ito init` with extend mode, multi-tool selection, and an interactive `AGENTS.md` configurator.

## 0.2.0

### Minor Changes

- ce5cead: - Add an `ito dashboard` command that rolls up spec counts and change progress at a glance
  - Generate and update AI slash commands alongside the renamed `ito/AGENTS.md` instructions file
  - Remove the deprecated `ito diff` command and direct users to `ito show`
