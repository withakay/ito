...
#[test]
fn config_set_get_supports_coordination_and_audit_mirror_keys() {
    ...
    let out = run_rust_candidate(
        rust_path,
        &["config", "set", "audit.mirror.enabled", "true"],
        repo.path(),
        home.path(),
    );
    assert_eq!(out.code, 0, "stderr={}", out.stderr);
    ...
    let out = run_rust_candidate(
        rust_path,
        &["config", "get", "audit.mirror.enabled"],
        repo.path(),
        home.path(),
    );
    assert_eq!(out.code, 0, "stderr={}", out.stderr);
    ...
    let out = run_rust_candidate(
        rust_path,
        &["config", "get", "audit.mirror.branch"],
        repo.path(),
        home.path(),
    );
    assert_eq!(out.code, 0, "stderr={}", out.stderr);
    assert_eq!(out.stdout.trim(), "ito/internal/custom-audit");
}
...
#[test]
fn config_set_rejects_invalid_audit_mirror_branch_name() {
    let base = fixtures::make_empty_repo();
    let repo = tempfile::tempdir().expect("work");
    let home = tempfile::tempdir().expect("home");
    let rust_path = assert_cmd::cargo::cargo_bin!("ito");
    ...
    let out = run_rust_candidate(
        rust_path,
        &["config", "set", "audit.mirror.branch", "bad..branch"],
        repo.path(),
        home.path(),
    );
    assert_ne!(out.code, 0);
    assert!(out.stderr.contains("Invalid value 'bad..branch'"));
    assert!(out.stderr.contains("audit.mirror.branch"));
}
